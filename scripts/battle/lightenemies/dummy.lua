...
function Dummy:init()
    ...
    self:setActor("lightbattle/dummy")
    ...
    local action_party = {}
    for _, party in ipairs({ "ralsei", "noelle" }) do
        if Game:hasPartyMember(party) then
            table.insert(action_party, party)
        end
    end
    if #action_party == 0 then action_party = { "noelle" } end
    self:registerAct("Tell Story", "", action_party)
...
function Dummy:onAct(battler, name)
    ...
    elseif name == "Tell Story" then
        ...
        return "* You and " .. member .. " told the dummy\na bedtime story.\n* The enemies became [color:blue]TIRED[color:reset]..."

    elseif name == "Attack" then
        self.wave_override = TableUtils.pick({ "basic", "aiming", "movingarena" })
        return "* ok"
    ...
    elseif name == "Wave Mode" then
        if self.encounter.wave_mode then
            ...
            self.encounter:createBackground()
            self.waves = {}
            return "* Normal Mode."
        else
