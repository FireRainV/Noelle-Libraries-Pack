...
function spell:init()
    ...
    -- Check description
    self.check = { "Launches a barrage of bullets targeting\none foe.", "* Depends on Attack & Magic." }
...
function spell:getDamage(user, target)
    if Game:isLight() then
        return math.ceil((user.chara:getStat("magic") * 2) + user.chara:getStat("attack") - target.defense) * 6
    else
        return math.ceil((user.chara:getStat("magic") * 3) + (user.chara:getStat("attack") * 1.5) - (target.defense * 2)) * 6
    end
end
