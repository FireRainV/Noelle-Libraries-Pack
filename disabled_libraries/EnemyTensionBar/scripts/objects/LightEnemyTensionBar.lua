...
function LightTensionBar:giveTension(amount)
...
function LightTensionBar:removeTension(amount)
    local start = self:getTension()
    self:setTension(self:getTension() - amount)
...
function LightTensionBar:setTension(amount, dont_clamp)
    self.tension = dont_clamp and amount or MathUtils.clamp(amount, 0, self.max_tension)
end

function LightTensionBar:getTension()
    return self.tension or 0
end

function LightTensionBar:setMaxTension(amount)
    self.max_tension = amount
end

function LightTensionBar:getMaxTension()
...
function LightTensionBar:flash()
    ...
    if self.current_flash == nil or self.current_flash:isRemoved() then
        self.current_flash = self:addChild(LightEnemyTensionBarGlow()) --[[@as TensionBarGlow]]
    else
...
function LightTensionBar:getDebugInfo()
    ...
    table.insert(info, "Tension: " .. MathUtils.round(self:getPercentageFor(self.tension) * 100) .. "%")
...
function LightTensionBar:getTension250()
    return self:getPercentageFor(self.tension) * 250
end
...
function LightTensionBar:getPercentageFor(variable)
    return variable / self.max_tension
end
