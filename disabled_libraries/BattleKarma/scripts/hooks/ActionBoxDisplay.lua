...
-- Add the karma bar (drawn below the health bar)
function ActionBoxDisplay:draw()
    ...
    Draw.setColor(Game.battle.encounter.karma_mode and KARMA_PALETTE["background"] or PALETTE["action_health_bg"])
    ...
    if health > 0 then
        Draw.setColor(KARMA_PALETTE["karma"])
